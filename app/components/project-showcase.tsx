...
const projectTemplates: ProjectTemplate[] = [
  ...
  },
  {
    ...
    title: "학습 아카이브",
    category: "개인 프로젝트 04",
    summary: "Notion 학습 노트를 검수한 뒤 정적 웹사이트로 자동 발행하는 개인 지식 아카이브 파이프라인입니다.",
    stack: ["Astro", "JavaScript", "Node.js", "Notion API", "GitHub Actions", "GitHub Pages", "Mermaid", "KaTeX", "CSS", "Git", "Claude Code"],
    tone: "lavender",
    readme: {
      overview: "강의와 자료로 배운 내용을 그때만 이해하고 흘려보내지 않기 위해 만든 개인 지식 아카이브입니다. 익숙한 Notion에서 학습 노트를 작성하고 검수한 뒤, 원하는 글만 빠르고 안정적인 정적 웹사이트로 공개합니다. 콜아웃·다이어그램·표 같은 시각 자료도 함께 반영해 다시 볼 때 이해와 복습 효율을 높였습니다.",
      siteUrl: "https://stux12.github.io/study-archive/",
      repositoryUrl: "https://github.com/stux12/study-archive",
      features: [
        "상태 기반 검수와 발행: Notion 글의 진행 중·Published·완료 상태로 노출을 제어해 검수를 통과한 글만 발행합니다.",
        "Notion 변환 파이프라인: 콜아웃·토글·표·코드 블록·Mermaid·수식·이미지·인라인 서식을 정적 사이트용 Markdown과 HTML로 변환합니다.",
        "커밋백 발행 모델: Published 글은 최신 내용으로 재생성하고 완료 글은 확정본으로 고정하며, 초안 전환과 삭제는 사이트에도 반영합니다.",
        "탐색 경험: 분야·카테고리·태그 3단계 분류와 클라이언트 검색·필터, 라이트·다크 테마를 제공합니다.",
        "발행 안전장치: dry-run 미리보기, 중복 슬러그·0건 가드, 내용 누락 경고와 직접 작성 글 보존 규칙을 적용했습니다.",
        "공개 환경: Open Graph·RSS·sitemap을 생성해 검색과 구독, 공유 경험까지 함께 관리합니다.",
      ],
      architecture: {
        description: "Notion을 콘텐츠 원본으로 두고 GitHub Actions가 검수된 글을 동기화·변환·영속화한 뒤, Astro가 정적 사이트를 빌드해 GitHub Pages에 배포하는 서버리스 파이프라인입니다.",
        choices: [
          { category: "사이트 생성", name: "Astro · GitHub Pages", reason: "읽기 위주·저트래픽인 개인 아카이브에는 상시 서버보다 정적 사이트가 적합했습니다. 운영 부담과 비용을 없애는 대신 내용 갱신 때 빌드가 필요합니다." },
          { category: "콘텐츠 원본", name: "Notion DB · Notion API", reason: "별도 CMS를 만들지 않고 익숙한 Notion에서 작성·검수하기 위해 헤드리스 CMS로 사용했습니다. 대신 Notion 블록을 Markdown으로 변환하는 로직을 직접 구현했습니다." },
          { category: "발행 제어", name: "상태 필터 · 커밋백", reason: "무상태 GitHub Actions 환경에서도 완료 글을 고정하려고 생성물을 저장소에 커밋합니다. 결과물이 쌓이는 비용 대신 재조회 없이 확정본을 유지합니다." },
          { category: "배포 안전장치", name: "workflow_dispatch", reason: "Notion에서 상태를 실수로 바꿔도 바로 공개되지 않도록 사람이 수동 실행으로 최종 승인합니다. 배포마다 실행 한 번이 필요한 것이 트레이드오프입니다." },
          { category: "시각 자료", name: "Mermaid · KaTeX · 변환기", reason: "글과 그림을 함께 기억하는 이중 부호화를 위해 다이어그램·수식·콜아웃을 렌더링합니다. 표현력 대신 클라이언트 번들 크기가 늘어납니다." },
          { category: "이미지 보존", name: "저장소 자산화", reason: "만료될 수 있는 Notion 이미지 URL 대신 이미지를 내려받아 저장소 자산으로 관리해 링크 깨짐과 재현성 문제를 줄였습니다." },
        ],
        diagram: { client: "작성자 · Notion · 검수 상태", api: "GitHub Actions · sync-notion.mjs", dependencies: ["Notion API · 블록·이미지 조회", "GitHub 저장소 · Markdown·이미지 커밋백", "Astro 정적 빌드 → GitHub Pages", "방문자 · 브라우저 열람"] },
        deployment: "GitHub Actions를 workflow_dispatch로 수동 실행하면 Notion 동기화·Markdown 변환·생성물 커밋백·Astro 빌드·GitHub Pages 배포가 순서대로 진행됩니다. dry_run 옵션으로 실제 배포 전 발행 대상과 결과를 미리 확인합니다. Notion 토큰은 GitHub Secrets로만 주입하고, 검수 체크리스트에 공개 가능 여부를 포함해 민감 정보 노출을 방지합니다.",
      },
      implementation: {
        overview: "‘Notion에서 작성하고 검수 후 정적 사이트로 공개’하는 흐름을 목표로, 콘텐츠 변환·발행 상태·무상태 CI의 영속성 문제를 단계적으로 해결했습니다.",
        steps: [
          { label: "01", title: "문제 정의", description: "학습 내용을 다시 정리·복습하지 않아 쉽게 휘발되는 문제를 정의하고, 기록과 공개를 분리하는 개인 아카이브 방향을 세웠습니다." },
          { label: "02", title: "파이프라인 설계", description: "Notion을 콘텐츠 원본, GitHub Actions를 발행 파이프라인, GitHub Pages를 호스팅으로 두고 글 상태로 발행을 제어하도록 설계했습니다." },
          { label: "03", title: "변환과 사이트 구현", description: "블록→Markdown 변환과 이미지 자산화를 구현하고, Astro에 디자인 토큰·테마·검색·필터·SEO를 구성했습니다." },
          { label: "04", title: "무상태 CI 문제 해결", description: "완료 글을 고정해야 하는 요구와 매번 초기화되는 CI가 충돌했습니다. 생성물을 커밋백해 Published=재생성·완료=고정·삭제=반영 규칙과 직접 작성 글 보호 로직을 정립했습니다." },
          { label: "05", title: "발행 검증", description: "dry-run으로 발행 대상을 확인하고, 로컬 Astro 빌드·중복 슬러그·0건 가드·내용 누락 경고와 라이브 페이지로 결과를 검증합니다." },
          { label: "06", title: "배포와 운영", description: "수동 트리거로 동기화부터 Pages 배포까지 통제하고, Notion 토큰은 GitHub Secrets에서만 관리합니다." },
        ],
        principles: [
          "작성과 발행을 분리하고 사람이 최종 승인해 자동화 과정의 오발행을 막습니다.",
          "무상태 CI의 한계를 이해하고 커밋백으로 확정 상태를 영속화합니다.",
          "자동 삭제에는 직접 작성 글 보존 규칙과 dry-run 미리보기를 함께 둡니다.",
          "비밀값은 GitHub Secrets로만 관리하고 저장소에는 넣지 않습니다.",
          "학습 글의 이해와 복습을 위해 시각 자료까지 변환하는 파이프라인을 구축합니다.",
        ],
      },
    },
    note: "해당 프로젝트는 Claude Code를 통해 개발된 프로젝트 입니다.",
